...
ENVFILE
    ...
    printf "     Uncomment ONE auth method:\n"
    printf "     ${DIM}- KIRO_CREDS_FILE  (Kiro IDE JSON token file)${RESET}\n"
    printf "     ${DIM}- REFRESH_TOKEN    (direct refresh token)${RESET}\n"
    printf "     ${DIM}- KIRO_CLI_DB_FILE (kiro-cli SQLite database)${RESET}\n"
    printf "  ${CYAN}2.${RESET} Restart terminal (or run: source ~/.bashrc)\n"
    printf "  ${CYAN}3.${RESET} Start the gateway: ${DIM}kiro-gateway${RESET}\n"
    printf "     ${DIM}(opens interactive menu: start/stop/status/login/configure)${RESET}\n"
    printf "\n"
    printf "  ${BOLD}Connect your client:${RESET}\n"
    printf "  ${DIM}Base URL:  ${RESET}http://localhost:2507/v1\n"
    printf "  ${DIM}API Key:   ${RESET}your PROXY_API_KEY value\n"
    printf "\n"
    printf "  ${BOLD}Example (OpenAI Python):${RESET}\n"
    printf "  ${DIM}client = OpenAI(base_url=\"http://localhost:2507/v1\", api_key=\"YOUR_KEY\")${RESET}\n"
    printf "  ${DIM}client.chat.completions.create(model=\"claude-sonnet-4\", messages=[...])${RESET}\n"
    printf "\n"
    printf "  ${BOLD}Example (curl):${RESET}\n"
    printf "  ${DIM}curl http://localhost:2507/v1/chat/completions \\\\${RESET}\n"
    printf "  ${DIM}  -H \"Authorization: Bearer YOUR_KEY\" \\\\${RESET}\n"
    printf "  ${DIM}  -d '{\"model\":\"claude-sonnet-4\",\"messages\":[{\"role\":\"user\",\"content\":\"hi\"}]}'${RESET}\n"
    printf "\n"
    ...
    printf "  ${DIM}Uninstall:            kiro-gateway (menu option 8) or rm -rf $INSTALL_DIR${RESET}\n"
    printf "  ${DIM}Docs:                 https://github.com/redok07/kiro-gateway#configuration${RESET}\n\n"
}
